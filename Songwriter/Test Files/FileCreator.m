...
[y1,Fs1] = audioread('Bass Edited.wav');
[y2,~] = audioread('Drums Combined.wav');
[y3,~] = audioread('Fiddle Combined.wav');
[y4,~] = audioread('GtrNMandolin Edited.wav');
[y5,~] = audioread('Vox Edited.wav');
...
for i=1:5
    if i == 1
        ...
        file1 = file1/x1;
        audiowrite('Songwriter_math.wav',file1,44100);
    elseif i == 2
        clear G
        ...
        file2 = file2/x2;
        audiowrite('Songwriter_spec.wav',file2,44100);
    elseif i == 3
        clear G
        ...
        file3 = file3/x3;
        audiowrite('Songwriter_prod.wav',file3,44100);
    elseif i == 4
        clear G                      
        file4 = Y.y1s+Y.y2s+Y.y3s+Y.y4s+Y.y5s;
        x4 = max(max(abs(file4)));
        file4 = file4/x4;
        audiowrite('Songwriter_unrev.wav',file4,44100);
    elseif i == 5
        LPF=dsp.LowpassFilter('PassbandFrequency',1000,'StopbandFrequency',1500);
        file5 = LPF(file4);
        x5 = max(max(abs(file5)));
        file5 = file5/x5;
        audiowrite('Songwriter_lpf.wav',file5,44100);
    end
end
